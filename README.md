...
Prepare data and start a detached L4 run:

```bash
python -m modal run modal_train.py --action benchmark
python -m modal run modal_train.py --action upload
python -m modal run modal_train.py --action prepare
python -m modal run --detach modal_train.py --action train --gpu L4 --batch-size 16
```

Evaluate and export after training:

```bash
python -m modal run modal_train.py --action evaluate
python -m modal run modal_train.py --action export
```

The Modal app and persistent Volume are named `haru` and `haru-training`.
T4 uses fp16 with gradient scaling; L4 uses bf16.
...
| `modal_train.py` | Modal benchmark, preparation, training, and export |
